...
from transformers import CLIPTokenizer, CLIPTextModel, PretrainedConfig, CLIPVisionModelWithProjection, CLIPImageProcessor, CLIPFeatureExtractor
...
def log_validation(validation_dataloader, vae, text_encoder, tokenizer, unet, args, accelerator, weight_dtype, epoch,split="val"):
    logger.info("Running {} validation... ".format(split))

    scheduler = DDIMScheduler.from_pretrained(args.pretrained_model_name_or_path, subfolder="scheduler", cache_dir=cache_dir)
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Prompts to use for validation. If `None`, use all prompts."
        ),
    )
    parser.add_argument(
...
def main(args):
    ...
    noise_scheduler = DDPMScheduler.from_pretrained(args.pretrained_model_name_or_path, subfolder="scheduler", revision=args.revision, cache_dir=cache_dir)
    text_encoder = CLIPTextModel.from_pretrained(args.pretrained_model_name_or_path, subfolder="text_encoder", revision=args.revision, cache_dir=cache_dir)
    ...
    unet = UNet2DConditionModel.from_pretrained(args.pretrained_model_name_or_path, subfolder="unet", revision=args.revision, cache_dir=cache_dir)
    ...
    train_dataset = TextImageDataset(dataset_path=args.train_data_dir, height=args.resolution, width=args.resolution)
    validation_dataset = TextImageDataset(dataset_path=args.train_data_dir, height=args.resolution, width=args.resolution)
